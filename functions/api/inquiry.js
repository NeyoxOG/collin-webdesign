...
export async function onRequestPost({request,env}){
  ...
  const details={contact_preference:clean(b.contact_preference,40),business_name:clean(b.business_name,120),current_website:clean(b.current_website,220),goal:clean(b.goal,1000),target_group:clean(b.target_group,500),style:clean(b.style,80),colors:clean(b.colors,200),page_scope:clean(b.page_scope,80),content_ready:clean(b.content_ready,80),deadline:clean(b.deadline,40),features:arr(b.features)};
  const id=crypto.randomUUID(),created=new Date().toISOString(),token=randomToken(),tokenHash=await hash(token+env.SESSION_SECRET);
  await env.DB.prepare('INSERT INTO inquiries(id,name,email,phone,project_type,pages_name,domain_help,message,details_json,chat_token_hash,project_status,updated_at,created_at,status,ip_hash) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').bind(id,name,email,phone,project,pages,domain,message,JSON.stringify(details),tokenHash,'requested',created,created,'new',ipHash).run();
  await env.DB.prepare('INSERT INTO messages(id,inquiry_id,sender,body,created_at) VALUES(?,?,?,?,?)').bind(crypto.randomUUID(),id,'system','Deine Anfrage ist angekommen. Du kannst deinen Projektstatus jederzeit über „Mein Projekt“ verfolgen und hier im Chat Ergänzungen senden.',created).run();
  const qs=`id=${encodeURIComponent(id)}&token=${encodeURIComponent(token)}`;
  return Response.json({ok:true,id,token,chat_url:`/chat/?${qs}`,project_url:`/project/?${qs}`},{headers:{'cache-control':'no-store'}})
}
